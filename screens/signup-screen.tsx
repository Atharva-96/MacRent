...
import { validateEmail, validatePassword, signInWithGoogle, signInWithFacebook } from "../utils/auth"
  ...
  const handleFacebookSignup = async () => {
    ...
    try {
      ...
      if (result.success) {
        // In a real app, you would send this token to your backend
        // For now, we'll simulate a successful signup
        await signup(result.user.name, result.user.email, "facebook-auth-token")
      } else {
        Alert.alert("Signup Failed", result.error || "Failed to signup with Facebook")
      }
    } catch (error) {
      Alert.alert("Error", "An error occurred during Facebook signup")
      console.error(error)
    } finally {
      setIsAuthenticating(false)
    }
